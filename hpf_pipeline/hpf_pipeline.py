#!/usr/bin/env python3
"""
PATH Pipeline: Data Preparation → Feature Engineering → Model Training → Evaluation → Visualization
"""
...
class PATHPipeline:
    ...
    def setup_directories(self):
        ...
        base_dir = Path(self.config.get("base_dir", "PATH"))
        self.dirs = {
            "base": base_dir,
    ...
    def run(self):
        """Execute the full PATH pipeline"""
        self.logger.info("Starting PATH pipeline")
        ...
        try:
            ...
            corr_filtered = processed_data[[self.config["label"], self.config["split_column"]] + corr_features]
            
            # Apply Boruta feature selection
            boruta_features = self.boruta_selection(corr_filtered)
            boruta_filtered = corr_filtered[[self.config["label"], self.config["split_column"]] + boruta_features]
            ...
            self.logger.info("PATH pipeline completed successfully")
...
def main():
    """Main function to run the PATH pipeline"""
    # Configuration
    config = {
        "seed": 0,
        "base_dir": "path_pipeline",
