#!/usr/bin/env python3
"""
RAD Pipeline: Data Preparation → Feature Engineering → Model Training → Evaluation → Visualization
"""
...
class RADPipeline:
    ...
    def setup_logging(self):
        ...
        self.logger = logging.getLogger("RAD")
        
    def setup_directories(self):
        """Create necessary directories for output"""
        base_dir = Path(self.config.get("base_dir", "RAD"))
        self.dirs = {
            "base": base_dir,
    ...
    def boruta_selection(self, data: pd.DataFrame) -> List[str]:
        ...
        if self.config["run_boruta"]:
            ...
            if self.config.get("keep_tentative", True):
    ...
    def run(self):
        """Execute the full RAD pipeline"""
        self.logger.info("Starting RAD pipeline")
        ...
        try:
            ...
            corr_filtered = processed_data[[self.config["label_column"], self.config["split_column"]] + corr_features]
            
            # Apply Boruta feature selection
            boruta_features = self.boruta_selection(corr_filtered)
            boruta_filtered = corr_filtered[[self.config["label_column"], self.config["split_column"]] + boruta_features]
            ...
            self.logger.info("RAD pipeline completed successfully")
...
def main():
    """Main function to run the RAD pipeline"""
    # Configuration
    config = {
        "seed": 2,
        "base_dir": "rad_pipeline",
